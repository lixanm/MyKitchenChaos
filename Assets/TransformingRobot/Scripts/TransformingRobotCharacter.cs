...
public class TransformingRobotCharacter : MonoBehaviour {
	...
	public void RobotModeChange(int aRobotMode){
		...
		if (robotMode == 0) {
			...
			robotRigidBody.constraints=RigidbodyConstraints.FreezeRotationZ | RigidbodyConstraints.FreezeRotationX;
			robotRigidBody.useGravity=true;
		}else if(robotMode==2){
			robotAnimator.applyRootMotion=false;
	...
	public void Move(float v,float h){
		...
		if (robotMode == 1) {
			robotRigidBody.AddForce(v*transform.forward*Time.deltaTime*tankSpeed,ForceMode.Force);
			robotRigidBody.AddTorque(h*transform.up*Time.deltaTime*tankRotateSpeed,ForceMode.Force);
		} else if (robotMode == 2) {	
			robotRigidBody.AddForce(transform.forward*Time.deltaTime*planeSpeed);
			robotRigidBody.AddTorque(h*transform.up*Time.deltaTime*planeRotateSpeed,ForceMode.Force);
			robotRigidBody.AddTorque(v*transform.right*Time.deltaTime*planeRotateSpeed,ForceMode.Force);
		}
	}
}
